...
const launchScannerCamera = () => {
  ...
  const sendPart1 = async () => {
    const res = await fetch(mainEndpoint + '/productimageversion/add-version', {
      method: 'POST',
      headers: {
  ...
  const sendPart2 = rId => {
     return fetch(mainEndpoint + '/productimageversion/update-part-version', {
      method: 'POST',
      headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json'
